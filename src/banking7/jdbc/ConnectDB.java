...
추상클래스는 객체를 생성할 수는 없지만 참조변수선언, 상속 등의 기능은 사용할 수 있다.
*/
public abstract class ConnectDB {
	
	//멤버변수: 상속관계에서 접근가능한 접근지정자로 지정함
	protected Connection con; //DB연경
	...
	*/
	public ConnectDB() {
		...
		try {
			...
			con = DriverManager.getConnection(
					"jdbc:oracle:thin:@localhost:1521:xe", "education", "1234");
		}
		catch (ClassNotFoundException e) {
			System.out.println("오라클 드라이버 로딩 실패");
		}
	...
	/*
	상속관계에서 오버라이딩을 목적으로 생성한 추상메서드로 하위클래스에서 각 업무(update, select 등)에따라 재정의 하게 될것이다.
	
	추상메서드를 포함한 클래스는 반드시 추상클래스로 선언해야 에러가 발생하지 않는다.
	따라서 해당 클래스는 abstract로 선언해야 한다.
	*/
